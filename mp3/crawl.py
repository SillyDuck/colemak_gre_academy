...
abrade,rubbed off worn away by friction      
...
acclaim,welcomed with shouts and approval       
...
coalesce,coming together and uniting into one substance     
...
flaunt,show off complacently         
flax,pale yellow (hair) a plant       
fledge,able to fly trained experienced       
...
impugn,challenged to be doubted        
...
opaque,dullness/not allowing light to pass through    
...
precept,rules establishing standards of conduct       
...
recital,a number of performance of music      
...
sully,to be stained or discredited       
...
import csv
lines = masterCSV.splitlines()
reader = csv.reader(lines, delimiter=',')
import requests
import string
mydict = {}
for c in string.ascii_lowercase:
    with open("dict/"+c+'.lib', 'r', encoding = "Big5", errors='replace') as f:
        b_str = f.readlines()
        for l in b_str:
            a = l.split('=')
            mydict[a[0].lower()] = a[1]
# mydict['abraded'] = "磨損的"
# mydict['acclaimed'] = "廣受好評的"
# mydict['chastened'] = "學乖了"
# mydict['coalescing'] = "凝聚"
mydict['denouement'] = "結局"
mydict['deviance'] = "偏常，異常；異常行為"
mydict['unencumbered'] = "無拘無束"
mydict['hoi'] = "hoi polloi, 庶民"
rows = []
with open('zch.csv', 'w',encoding = "utf-8",errors='replace') as csvFile:
    for row in reader:
        # url = "http://s.cdict.net/{}/{}.mp3".format(row[0][0],row[0])
        # r = requests.get(url)
        ...
        # else:
        #     print(row[0])
        #     r = requests.get("https://s.yimg.com/bg/dict/dreye/live/m/{}.mp3".format(row[0]))
        #     if r.status_code == requests.codes.ok:
        #         f = open(row[0]+".mp3","wb")
        #         f.write(r.content)
        #         f.close()
        ...
        print(row[0],row[1],mydict[row[0]])
        #if row[0] not in mydict:
        #    print(row[0])
        
        #row[1] = row[1].encode('utf-8')
        row.append(mydict[row[0]].replace(',',' '))
        #row[0] = row[0].encode('utf-8')
        #rows.append(row)
        #csvFile.write(row[0]+b','+row[1]+b','+row[2]+b'\n')
        csvFile.write(','.join(row)+'\n')
    #print()

# with open('zch.csv', 'w', newline='') as csvFile:
